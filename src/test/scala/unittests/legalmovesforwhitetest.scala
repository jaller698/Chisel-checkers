...
//        dut.io.In:=VecInit(Seq.tabulate(32) { i =>
      ...
      for (i <- 0 to 32 * 4 - 1) { // -1 because scala's range is shit.
        dut.io
          ...
          .expect(
            false.B,
            s" $i should be 0, because the board is empty and thus no moves are possible!"
          )
      }
      ...
      for (i <- 0 to 32 * 4 - 1) { // -1 because scala's range is shit.
        if (i == 14 * 4 || i == 14 * 4 + 2) {
          dut.io
            ...
            .expect(
              true.B,
              s" $i should be true, because the piece can jump from here"
            )
      ...
      for (i <- 0 to 32 * 4 - 1) { // -1 because scala's range is shit.
        if (i == 26 * 4 + 3) {
          ...
          dut.io
            ...
            .expect(
              true.B,
              s" $i should be true, because the piece can go to the left from here"
            )
        } else {
          dut.io
            .whereWeCanMove(i)
