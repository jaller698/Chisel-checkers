...
class whiteforcedmovestest extends AnyFlatSpec with ChiselScalatestTester {
  ...
  it should "have a forced move with a white pawn on 26 and a black pawn on 23" in {
    test(new WhiteForcedMoves()) { dut =>
      for (i <- 0 to 31) {
        if (i == 26) {
          dut.io.In(i).poke("b001".U) // white pawn
        } else if (i == 23) {
          dut.io.In(i).poke("b011".U) // black pawn to jump over
        } else {
          dut.io.In(i).poke("b000".U) // empty
        }
      }
